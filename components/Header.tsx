...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg' : 'bg-white shadow-sm'}`}>
      {/* Contact Bar - hidden when scrolled for focus */}
      ...
      {/* Main Navigation */}
      <nav className={`transition-all duration-300 ${isScrolled ? 'bg-transparent' : 'bg-white'}`}>
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center py-5 transition-all duration-300">
            {/* Logo */}
            <Link href="/" className="flex items-center gap-4 group">
              ...
              <div>
                <div className="font-black tracking-tighter text-slate-900 transition-all duration-300 text-xl">
                  {siteInfo.title}
                </div>
            ...
            {/* Mobile Menu Button */}
            <button
              className="lg:hidden w-12 h-12 flex flex-col justify-center items-center gap-1.5 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-all active:scale-90"
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            >
              <span className={`w-6 h-0.5 bg-slate-900 transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
              <span className={`w-6 h-0.5 bg-slate-900 transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
              <span className={`w-6 h-0.5 bg-slate-900 transition-all duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
            </button>
          </div>
        </div>
        ...
        {/* Mobile Menu Overlay */}
        <div className={`lg:hidden fixed inset-0 z-[-1] bg-white/95 backdrop-blur-2xl transition-all duration-500 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
          <div className="container mx-auto px-4 pt-24 pb-12 h-full flex flex-col overflow-y-auto">
                  ...
                  <div key={index} className="animate-fade-in-up" style={{ animationDelay: `${index * 50}ms` }}>
                    {isExternal ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-between w-full p-6 bg-[#0070af] text-white rounded-[2rem] font-black text-lg shadow-xl shadow-[#0070af]/20"
                      ...
                      <div className="space-y-2">
                        <Link
                          href={item.href}
                          className="block p-6 text-3xl font-black text-slate-900 hover:text-[#0070af] transition-colors"
                          onClick={() => setIsMobileMenuOpen(false)}
                        >
                          {item.title}
                        </Link>
                          ...
                          <div className="grid grid-cols-1 gap-2 ml-6">
                            {item.submenu.map((subitem, subindex) => (
                              <Link
                                key={subindex}
                                href={subitem.href}
                                className="block py-3 px-6 text-lg font-bold text-slate-600 border-l-2 border-slate-100 hover:border-[#0070af] hover:text-[#0070af] transition-all"
                                onClick={() => setIsMobileMenuOpen(false)}
                              >
                                {subitem.title}
                              </Link>
            ...
            {isAdminLoggedIn && (
              <div className="mt-auto pt-12 border-t border-slate-100 flex flex-col gap-4">
                <Link
                  href="/admin"
                  className="flex items-center justify-center gap-3 p-6 bg-slate-900 text-white rounded-[2rem] font-black text-xl"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                ...
                <button
                  ...
                  className="p-6 text-slate-500 font-black uppercase tracking-widest text-sm"
                >
                  Odhlásit se
                </button>
              </div>
