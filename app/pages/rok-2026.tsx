...
export default function Rok2026Page() {
  ...
  useEffect(() => {
    const fetchGalleries = async () => {
      try {
        ...
        if (result.success) {
          const galleries2026 = result.data.galleries.filter((g: PhotoGallery) =>
            new Date(g.date).getFullYear() === 2026
          );
          setGalleries(galleries2026);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Rok 2026</h1>
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-900 mb-2">Rok 2026</h1>
      <p className="text-lg text-gray-700 mb-8">
        Fotogalerie a vzpomínky z akcí roku 2026.
      </p>

      {galleries.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          <p className="text-gray-500 text-lg">Zatím žádné fotogalerie z roku 2026.</p>
        </div>
      ) : (
                ...
                <>
                  ...
                  {gallery.photos.length > 8 && (
                    <div className="p-4 border-t bg-gray-50">
                      ...
                      >
                        Zobrazit všechny fotky ({gallery.photos.length})
                      </button>
                    </div>
                  )}
                </>
        ...
        >
          <div className="relative max-w-6xl w-full h-full flex items-center justify-center p-4">
            ...
            <div className="absolute bottom-4 left-0 right-0 text-center text-white">
              {lightboxPhoto.caption && <p className="mb-2">{lightboxPhoto.caption}</p>}
